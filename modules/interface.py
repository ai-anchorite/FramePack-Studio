...
def create_interface(
    ...
    def create_latents_layout_update():
        ...
        return gr.update(visible=display_top), gr.update(visible=not display_top, value=None if display_top else gr.update())
    ...
    with block:
        ...
        with gr.Row(elem_id="fixed-toolbar"):
            gr.HTML(f"""<div style="display: flex; align-items: center;"><h1 class='toolbar-title'>FP Studio</h1><p class='toolbar-version'>{APP_VERSION_DISPLAY}</p><p class='toolbar-patreon'><a href='https://patreon.com/Colinu' target='_blank'>Support on Patreon</a></p></div>""")
            queue_stats_display = gr.Markdown("<p style='margin:0;color:white;' class='toolbar-text'>Queue: 0 | Running: 0 | Completed: 0</p>")
            toolbar_ram_display_component = gr.Textbox(value="RAM: N/A", interactive=False, lines=1, max_lines=1, show_label=False, container=False, elem_id="toolbar-ram-stat", elem_classes="toolbar-stat-textbox")
            toolbar_vram_display_component = gr.Textbox(value="VRAM: N/A", interactive=False, lines=1, max_lines=1, show_label=False, container=False, elem_id="toolbar-vram-stat", elem_classes="toolbar-stat-textbox")
            toolbar_gpu_display_component = gr.Textbox(value="GPU: N/A", interactive=False, lines=1, max_lines=1, show_label=False, container=False, elem_id="toolbar-gpu-stat", elem_classes="toolbar-stat-textbox")

        # --- Tabs ---
        with gr.Tabs(elem_id="main_tabs") as main_tabs_component:
            with gr.Tab("Generate", id="generate_tab"):
                g = create_generate_ui(lora_names, default_prompt, DUMMY_LORA_NAME, get_latents_display_top, settings)
            ...
            with gr.Tab("Settings", id="settings_tab"):
                s = create_settings_ui(settings, get_latents_display_top, g["model_type"].choices)
        ...
        def update_start_button_state(*args):
            ...
            if is_video_model(selected_model) and not video_provided:
                return gr.update(value="❌ Missing Video", interactive=False), gr.update(visible=True)
        ...
        def apply_startup_settings():
            ...
            ui_components_updates_list = [gr.update() for _ in g.get("ui_components", [])]
        ...
        # --- Connect Events ---
        functions = {
            "is_video_model": is_video_model, "get_latents_display_top": get_latents_display_top,
            "create_latents_layout_update": create_latents_layout_update, "update_stats": update_stats,
            "check_for_current_job": check_for_current_job, "check_for_current_job_and_monitor": check_for_current_job_and_monitor,
            "end_process_with_update": end_process_with_update,
            "update_start_button_state": update_start_button_state, "apply_startup_settings": apply_startup_settings,
            "process_fn": process_fn, "monitor_fn": monitor_fn, "end_process_fn": end_process_fn,
            "xy_plot_process": xy_plot_process, "job_queue": job_queue, "settings": settings,
            "DUMMY_LORA_NAME": DUMMY_LORA_NAME, "block": block
        }
